...
diz_output={}
for belief_state_size in [200,300,400]:
    for state_size in [4,6,20]:
        #### load trained model
        checkpoint_2layer = torch.load("./output/model/new_model_epoch_99_belief_{}_state_{}_clock.pt".format(belief_state_size,state_size),map_location=torch.device('cpu'))
        input_size = 3
        processed_x_size = 3
    ...
    for belief_state_size in [200,300,400]:
        ...
        for state_size in [4,6,20]:
           ...
           # pred_roll=diz_output['output_spezzato_belief_{}_state_{}'.format(belief_state_size,state_size)][i,:,:]
            ...
            axs[asse1,asse2].plot(range(32),R_or, color='pink',label='R')
            axs[asse1,asse2].plot(range(t2,33),R_pred,color='purple',label='R_pred')
            #axs[asse1,asse2].set_title('Belief = {}, state = {}'.format(belief_state_size,state_size))
            asse2=asse2+1
        asse1=asse1+1
            
    axs[0,2].legend(loc='upper right',bbox_to_anchor=(1.4, 1))