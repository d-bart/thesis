...
#### preparing dataset
with open("C:/Users/barts/università/Tesi magistrale/Datasets_Generation/Datasets_Generation/data/SIR/SIR_training_set_test.pickle", 'rb') as file_handle:
...
for belief_state_size in [10,50,100,200]:
    for state_size in [2,4,8,10,15]:
        ...
        num_epoch = 100
        log_file_handle = open("./log/loginfo_new_belief_{}_state{}_SIR.txt".format(belief_state_size,state_size), 'w')
        for epoch in range(num_epoch):
            ...
            for idx, sequence in enumerate(data_loader):        
                ...
                t_1 = np.random.choice(15)
                t_2 = t_1 +  np.random.choice([1,2,3,4,5])
                loss = tdvae.calculate_loss(t_1, t_2)[0]
                dizionario_SIR['belief_{}_state_{}'.format(belief_state_size,state_size)]=tdvae.calculate_loss(t_1, t_2)[1]
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
            ...
            if (epoch + 1) % 50 == 0:
                torch.save({
                    ...
                    'loss': loss
                    }, "./output/model/new_model_epoch_{}_belief_{}_state_{}_SIR.pt".format(epoch,belief_state_size,state_size))