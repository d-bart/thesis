...
num_epoch = 4000
log_file_handle = open("./log/loginfo_new_decoder_dblock_SI_random.txt", 'w')
for epoch in range(num_epoch):
    ...
    for idx, sequence in enumerate(data_loader):        
        ...
        dizionario_100_epoch_2_layer_decoder_dblock_SI_random=tdvae.calculate_loss(t_1, t_2)[1]
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
    ...
    if (epoch + 1) % 50 == 0:
        torch.save({
            ...
            'loss': loss
        }, "./output/model/new_model_epoch_{}_lstm2layer_decoder_dblock_SI_random.pt".format(epoch))
