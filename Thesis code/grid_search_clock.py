...
dizionario_clock={}
for belief_state_size in [10,50,100,200]: #ultima prova: [200,300,400]
    for state_size in [2,4,8,10,15]: #ultima prova [4,6,20]
        ...
        num_epoch = 100 #ultima prova con 200
        log_file_handle = open("./log/loginfo_new_belief_{}_state{}_clock.txt".format(belief_state_size,state_size), 'w')
        for epoch in range(num_epoch):
            ...
            for idx, sequence in enumerate(data_loader):        
                ...
                dizionario_clock['belief_{}_state_{}'.format(belief_state_size,state_size)]=tdvae.calculate_loss(t_1, t_2)[1]
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
                ...
                print("epoch: {:>4d}, idx: {:>4d}, loss: {:.2f}, belief: {}, state: {}".format(epoch, idx, loss.item(),belief_state_size,state_size))
            ...
            if (epoch + 1) % 50 == 0:
                torch.save({
                    ...
                    'loss': loss
                    }, "./output/model/new_model_epoch_{}_belief_{}_state_{}_clock.pt".format(epoch,belief_state_size,state_size))